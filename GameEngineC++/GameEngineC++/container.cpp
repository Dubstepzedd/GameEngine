...
int Container::run() {
	...
	int code2 = Window::getInstance().start("Test", 520, 480, false, true);
	if (code == 1)
		return 1;
	...
	std::cout << "Resoultion: " << Window::getInstance().getResolutionX() << ":" << Window::getInstance().getResolutionY() << std::endl;
	std::cout << "Refresh rate: " << Window::getInstance().getRefreshRate() << std::endl;
	float cd = 0;
